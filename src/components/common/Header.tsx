...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-xs">
      ...
      <div className="bg-[#0b241b] text-slate-200 text-xs py-1.5 px-4 sm:px-8 flex flex-wrap items-center justify-between gap-2 border-b border-emerald-950">
        <div className="flex items-center gap-2 font-medium tracking-wide">
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-semibold text-slate-100">{t.govHeader}</span>
          </div>
          ...
          <span className="hidden lg:inline text-emerald-300 font-mono text-[11px]">e-Sanjeevani & ABDM Integrated</span>
        </div>
        ...
        <div className="flex items-center gap-3 text-xs ml-auto">
          ...
          >
            ...
            <span className="hidden sm:inline">{isOnline ? 'HMIS Live' : `Offline (${pendingSyncCount})`}</span>
          </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium leading-none">
              Maharashtra Rural Health Coordination & Telemedicine Platform
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Search ABHA / Patient Record</span>
          ...
          <div className="relative">
            ...
            >
              ...
              <span>
                {currentView === 'landing' ? 'Select Portal Console' : getRoleTitle(currentView as Role)}
              </span>
              <ChevronDown className="w-3.5 h-3.5 text-emerald-300" />
            </button>

            {roleDropdownOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-200 p-2 text-slate-800 z-50">
                <div className="px-3 py-2 border-b border-slate-100 text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                  <span>Authorized Health Portals</span>
                  <button 
          ...
          {currentView !== 'landing' && (
            <button
              onClick={() => setCurrentView('landing')}
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-2 rounded-xl text-xs font-semibold border border-slate-200 transition-colors"
            >
              Exit to Home
            </button>
          )}
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-base text-slate-900">ABHA & Patient Record Directory</h3>
              </div>
              <button 
            ...
            <form onSubmit={handleSearchPatient} className="flex gap-2">
              <input
                type="text"
                placeholder="Enter ABHA ID (e.g. 91-4821) or Mobile or Name..."
                value={searchQuery}
            ...
            <div className="text-[11px] text-slate-500">
              Try sample queries: <span className="font-mono font-bold text-slate-700 cursor-pointer underline" onClick={() => setSearchQuery('Sunita')}>Sunita</span>, <span className="font-mono font-bold text-slate-700 cursor-pointer underline" onClick={() => setSearchQuery('98230 44512')}>98230 44512</span>, <span className="font-mono font-bold text-slate-700 cursor-pointer underline" onClick={() => setSearchQuery('Shantabai')}>Shantabai</span>, <span className="font-mono font-bold text-slate-700 cursor-pointer underline" onClick={() => setSearchQuery('Ganesh')}>Ganesh</span>
            </div>
            ...
            {searchedPatient === 'NOT_FOUND' && (
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-center text-xs text-amber-800">
                No matching ABHA or patient record found for "{searchQuery}". Please check the spelling or create a new registration in the ASHA or CHO portal.
              </div>
            )}
